...
const AddHotelForm = () => {
  ...
  const removeGalleryPhoto = (index) => {
    ...
    if (galleryInputRef.current && gallery.length === 0)
  ...
  const handleRoomImageChange = (index, e) => {
    ...
    setHotelData((prev) => {
      ...
      roomsCopy[index].roomImages = [...(roomsCopy[index].roomImages || []), ...files];
      roomsCopy[index].roomPreviews = [...(roomsCopy[index].roomPreviews || []), ...previews];
      return { ...prev, rooms: roomsCopy };
    });
  };
  ...
  const removeRoomImage = (roomIndex, imageIndex) => {
    setHotelData((prev) => {
      const roomsCopy = [...prev.rooms];
      roomsCopy[roomIndex].roomImages.splice(imageIndex, 1);
      roomsCopy[roomIndex].roomPreviews.splice(imageIndex, 1);
      return { ...prev, rooms: roomsCopy };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const { name, city, address, mapEmbedUrl, policies, basePricePerNight, starRating } =
      hotelData;
    ...
    try {
      ...
      // --- ✅ Fix for room images mapping ---
      const roomsForJson = hotelData.rooms.map((room) => {
        const { roomImages, roomPreviews, ...roomWithoutImages } = room;
        return roomWithoutImages;
      });

      const roomIndices = [];
      hotelData.rooms.forEach((room, index) => {
        room.roomImages.forEach((file) => {
      ...
      formData.append("rooms", JSON.stringify(roomsForJson));
  ...
  return (
    <div className="addHotelFormContainer">
      ...
      <form onSubmit={handleSubmit} className="addHotelForm">
        ...
        <label>Lowest Room Price Per Night:<span className="required">*</span></label>
        <input
          type="number"
        ...
        {hotelData.rooms.map((room, index) => (
          <div key={index} className="room-section">
            ...
            <label>Basic Guest Quantity:<span className="required">*</span></label>
            <input
              type="number"
            ...
            <label>Extra Guest Price Per Night:<span className="required">*</span></label>
            <input
              type="number"
            ...
            <label>Available Rooms:<span className="required">*</span></label>
            <input
              type="number"
              name="availableRooms"
            ...
            <div className="gallery-preview">
              {room.roomPreviews.map((preview, i) => (
                <div key={i} className="image-preview">
                  <img src={preview} alt={`Room ${index + 1} - ${i + 1}`} />
                  <button type="button" onClick={() => removeRoomImage(index, i)}>
                    ×
                  </button>
                </div>
              ))}
            ...
            <button type="button" className="remove-room-btn" onClick={() => removeRoom(index)}>
              ❌ Remove Room
            </button>
          </div>
        ))}
        ...
        <div className="gallery-preview">
          {galleryPreviews.map((preview, index) => (
            <div key={index} className="image-preview">
              ...
              <button type="button" onClick={() => removeGalleryPhoto(index)}>×</button>
            </div>
          ))}
        </div>
...
//           availableRooms: "",
//           roomAmenities: [],
//           roomImages: [], // 🆕 stores uploaded image files
//           roomPreviews: [], // 🆕 stores preview URLs
//         },
//       ],
//   });
...
//   setHotelData((prev) => {
//     const roomsCopy = [...prev.rooms];
//     roomsCopy[roomIndex].roomImages = roomsCopy[roomIndex].roomImages.filter((_, i) => i !== imageIndex);
//     roomsCopy[roomIndex].roomPreviews = roomsCopy[roomIndex].roomPreviews.filter((_, i) => i !== imageIndex);
//     return { ...prev, rooms: roomsCopy };
//   });
// };
...
// hotelData.rooms.forEach((room, index) => {
//   room.roomImages.forEach((file) => {
//     formData.append("roomPhotos", file); // ✅ consistent field name
//   });
// });
