...
// // ... imports (React, useState, useEffect, useRef, useContext, HotelCard, Filters, etc.)
...
//   const [hotels, setHotels] = useState([]); // always array
//   const [filteredHotels, setFilteredHotels] = useState([]);
//   const [filters, setFilters] = useState({ propertyType: "All", price: "none", maxPrice: 20000, rating: "All" });
...
//       const res = await fetch(`http://localhost:3000/api/admin/hotels/city/${city}`);
...
//       // ✅ Wrap single hotel in array if needed
//       // const hotelsArray = Array.isArray(data.data) ? data.data : data.hotel ? [data.hotel] : [];
...
//         // propertyType: hotel.propertyType || "Other",
//       }));
...
//   useEffect(() => { if (city) fetchHotels(); }, [city]);
...
//           filterDataRef.current && !filterDataRef.current.contains(event.target)) {
...
//         <div ref={filterDataRef} className={`filters-wrapper ${showFilters ? "show" : ""}`}>
...
//             <h4 ref={filterIconRef} className="filterIcon" onClick={toggleFilters} style={{ cursor: "pointer" }}>
...
//              filteredHotels.length > 0 ? filteredHotels.map(hotel => <HotelCard key={hotel.id} hotel={hotel} />) :
//              <p>No hotels found</p>}
//           </div>
//         </div>
...
const HotelList = () => {
  ...
  const fetchHotels = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/user/hotelList/city/${city}`); 
      if (!res.ok) throw new Error(`Server responded with ${res.status}`);
